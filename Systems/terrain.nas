...
if (info != nil) {
	if (info[0] != nil) {
		...
		var terrain_height = info[0];
		print("TERRAIN ",terrain_height);
	}
	if (info[1] != nil) {
		if (info[1].solid !=nil) {
		...
		if (info[1].light_coverage !=nil) setprop("fdm/jsbsim/environment/terrain-light-coverage",info[1].light_coverage);
		if (info[1].load_resistance !=nil) setprop("fdm/jsbsim/environment/terrain-load-resistance",info[1].load_resistance);
		if (info[1].friction_factor !=nil) setprop("fdm/jsbsim/environment/terrain-friction-factor",info[1].friction_factor);
		if (info[1].bumpiness !=nil) setprop("fdm/jsbsim/environment/terrain-bumpiness",info[1].bumpiness);
		if (info[1].rolling_friction !=nil) setprop("fdm/jsbsim/environment/terrain-rolling-friction",info[1].rolling_friction);
		if (info[1].names !=nil) setprop("fdm/jsbsim/environment/terrain-names",info[1].names[0]);
		...
		if (terrain_name == "Ocean" and terrain_height >  min_carrier_alt)
